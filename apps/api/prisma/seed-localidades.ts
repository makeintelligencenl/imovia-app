/**
 * Seed de Estados e Cidades (fonte: IBGE)
 * Estados: todos os 27 (26 estados + DF)
 * Cidades: apenas Minas Gerais (853 municípios) para o MVP
 */
...
// 853 municípios de Minas Gerais (IBGE)
const CIDADES_MG = [
...
async function main() {
  ...
  for (const estado of ESTADOS) {
    await prisma.estado.upsert({
      where: { id: estado.id },
      update: { sigla: estado.sigla, nome: estado.nome },
      create: estado,
    })
  }
  ...
  // Buscar ID do MG
  const mg = await prisma.estado.findUnique({ where: { sigla: 'MG' } })
  if (!mg) throw new Error('Estado MG não encontrado')

  console.log('⏳ Inserindo cidades de MG...')

  // Inserir em lotes de 100 para não sobrecarregar
  const BATCH = 100
  let ibgeId = 3100000 // base fictícia — usamos índice sequencial
  let inseridos = 0

  for (let i = 0; i < CIDADES_MG.length; i += BATCH) {
    const lote = CIDADES_MG.slice(i, i + BATCH)
    await prisma.$transaction(
      lote.map((nome, j) =>
        prisma.cidade.upsert({
          where: { id: ibgeId + i + j },
          update: { nome, estadoId: mg.id },
          create: { id: ibgeId + i + j, nome, estadoId: mg.id },
        }),
      ),
    )
    inseridos += lote.length
    process.stdout.write(`\r  ${inseridos}/${CIDADES_MG.length}`)
  }

  console.log(`\n✅ ${inseridos} cidades de MG inseridas`)
}
